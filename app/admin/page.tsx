      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 lg:px-8 py-6 sm:py-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6 sm:mb-8">
          <div>
            ...
            <p className="text-[#8b949e] mt-1 text-sm">Manage your library</p>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
          <div className="bg-[#161b22] border border-[#30363d] p-3 sm:p-5 rounded-xl">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#00a8e1]" fill="currentColor" viewBox="0 0 24 24"><path d="M18.4 10.6C16.55 8.99 14.15 8 11.99 8c-4.65 0-8.58 3.03-9.96 7.22L3.9 16c1.05-3.19 4.05-5.5 7.6-5.5 1.95 0 3.73.72 5.12 1.88L13 16h9V7l-3.6 3.6z"/></svg>
              <p className="text-[#8b949e] text-xs sm:text-sm">Movies</p>
            </div>
            <p className="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2">{stats.totalMovies}</p>
          </div>
          <div className="bg-[#161b22] border border-[#30363d] p-3 sm:p-5 rounded-xl">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#a371f7]" fill="currentColor" viewBox="0 0 24 24"><path d="M21 3H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H3V5h18v14zM9 8h2v8H9zm4 0h2v8h-2z"/></svg>
              <p className="text-[#8b949e] text-xs sm:text-sm">Series</p>
            </div>
            <p className="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2">{stats.totalSeries}</p>
          </div>
          <div className="col-span-2 sm:col-span-1 bg-[#161b22] border border-[#30363d] p-3 sm:p-5 rounded-xl">
            <div className="flex items-center gap-2">
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-[#f0b429]" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
              <p className="text-[#8b949e] text-xs sm:text-sm">Rating</p>
            </div>
            <p className="text-2xl sm:text-3xl font-bold mt-1 sm:mt-2 text-[#f0b429]">★ {stats.avgRating}</p>
          </div>
        </div>
          ...
          <table className="w-full min-w-[600px]">
            <thead className="bg-[#21262d]">
              <tr>
                ...
                <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-[#8b949e] uppercase">Content</th>
                <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-[#8b949e] uppercase">Type</th>
                <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-[#8b949e] uppercase hidden sm:table-cell">Quality</th>
                <th className="px-3 sm:px-4 py-3 text-left text-xs font-medium text-[#8b949e] uppercase hidden sm:table-cell">Sources</th>
                <th className="px-3 sm:px-4 py-3 text-right text-xs font-medium text-[#8b949e] uppercase">Actions</th>
              </tr>
            </thead>
                ...
                <tr key={item.id} className="hover:bg-[#21262d]/50 transition-colors">
                  ...
                  <td className="px-3 sm:px-4 py-3">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <div className="w-10 h-14 sm:w-12 sm:h-16 rounded overflow-hidden bg-[#21262d] shrink-0">
                        {item.poster && (
                          <Image src={item.poster} alt={item.title} width={48} height={64} className="w-full h-full object-cover" />
                        )}
                      </div>
                      <div className="min-w-0">
                        <p className="font-medium truncate max-w-[120px] sm:max-w-none">{item.title}</p>
                        <p className="text-xs sm:text-sm text-[#8b949e]">{item.releaseDate.split('-')[0]} • ★ {item.rating}</p>
                      </div>
                    </div>
                  </td>
                  <td className="px-3 sm:px-4 py-3">
          ...
          <div className="bg-[#161b22] border border-[#30363d] rounded-2xl w-full max-w-lg shadow-2xl max-h-[90vh] overflow-y-auto">
            <form onSubmit={saveItem}>
              <div className="p-4 sm:p-6 border-b border-[#30363d]">
                <h2 className="text-lg sm:text-xl font-bold">Edit {editingItem.mediaType === 'movie' ? 'Movie' : 'Series'}</h2>
              </div>
              <div className="p-4 sm:p-6 space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 sm:px-4 py-2 bg-[#0d1117] border border-[#30363d] rounded-lg text-white focus:outline-none focus:border-[#00a8e1]"
                  />
                </div>
                <div>
              ...
              <div className="p-4 sm:p-6 border-t border-[#30363d] flex gap-2 sm:gap-3">
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 px-3 sm:px-4 py-2 bg-[#00a8e1] hover:bg-[#00b4e6] rounded-lg font-medium disabled:opacity-50 transition-all text-sm"
                >
                  {saving ? 'Saving...' : 'Save'}
                </button>
                <button
                  ...
                  className="px-3 sm:px-4 py-2 bg-[#21262d] hover:bg-[#30363d] border border-[#30363d] rounded-lg transition-all text-sm"
                >
                  Cancel
                </button>
              </div>
